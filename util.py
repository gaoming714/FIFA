...
def show_local(path=Path("./downloads")):
    if check_folder(path):
        logger.info("Folder OK {}".format(path))
    else:
        return []
    mp4_files = list(path.glob("*.mp4"))  # with folder
    video_ids = []
...
def check_folder(path, mkdir=False):
    if path.exists():
        ...
        if mkdir:
            logger.info("Mk Folder {}".format(path))
            os.makedirs(path)
            return True
        else:
            logger.warning("No Folder {}".format(path))
            return False
